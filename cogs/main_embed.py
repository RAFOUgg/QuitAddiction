...
class MainEmbed(commands.Cog):
    ...
    async def on_interaction(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if custom_id in ["nav_main_menu", "nav_stats"]: # nav_stats est un alias pour le retour
                await interaction.edit_original_response(embed=self.generate_dashboard_embed(player, interaction.guild), view=MainMenuView())
                ...
                await interaction.edit_original_response(embed=self.generate_dashboard_embed(player, interaction.guild), view=ActionsView(player))
            elif custom_id == "nav_phone":
                embed = self.generate_dashboard_embed(player, interaction.guild) # Affiche le téléphone PAR-DESSUS le dashboard
                ...
                if custom_id in ["action_smoke", "action_drink", "action_eat"]:
                    ...
                    if action_image_url:
                        action_embed = self.generate_dashboard_embed(player, interaction.guild)
                ...
                db.refresh(player)
                final_embed = self.generate_dashboard_embed(player, interaction.guild)
